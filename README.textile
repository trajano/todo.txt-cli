...
h1. Changes from original

* Added ^tag support.  This character is used instead of "#" because "#" gets treated as the beginning of a comment.
* Tags, Contexts and Projects are forced to be lowercase (when doing lsprj, lsc, lst).  This makes it easier for lookups.
* Added edit command which opens up a specific task into the EDITOR to allow the user to make changes to a task
* Added support for describing Tags, Contexts, Projects
* Added portable support from https://github.com/ginatrapani/todo.txt-cli/pull/22
